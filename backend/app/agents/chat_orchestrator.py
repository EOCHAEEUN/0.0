...
def safety_check_summary_node(state: FactofitState) -> FactofitState:
    ...
    if not (company_id and equipment_id):
        _response_payload(
            state,
            text="설비를 먼저 선택해주세요.",
            cards=[],
    ...
    items = SafetyCheckImprovementService.get_by_company_and_equipment(
        db, UUID(company_id), UUID(equipment_id)
    )
    ...
    if total_count == 0:
        ...
        planned_count = sum(1 for item in items if _as_text(item.get("improvement_plan")))
        unplanned_count = total_count - planned_count
        text = (
            f"해당 설비에 등록된 안전점검 항목은 총 {total_count}건이며, "
            f"이 중 향후 관리 계획이 작성된 항목은 {planned_count}건, "
            f"미작성 항목은 {unplanned_count}건입니다. "
            "점검 증빙과 향후 관리 계획은 신청서 탭에서 설비별로 등록하시면, "
            "신청서 작성 시 자동으로 반영됩니다."
        )
    ...
    _response_payload(
        state,
        text=text,
        cards=[{"type": "safety_check_summary", "data": {"total": total_count}}],
        intent="safety",
        answer_source="database",
    )
    return state
