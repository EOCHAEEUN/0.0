...
def get_glossary_csv_path() -> Path:
    for path in _csv_paths():
        if path.exists():
            return path
    raise FileNotFoundError("term_glossary_rows.csv 파일을 찾을 수 없습니다:\n" + "\n".join(map(str, _csv_paths())))


@lru_cache(maxsize=1)
def load_term_glossary() -> dict[str, TermRow]:
    rows = {}
    with get_glossary_csv_path().open("r", encoding="utf-8-sig", newline="") as f:
...
def health() -> dict[str, Any]:
    ...
    return {"ok": True, "router": "click_chat_two", "csv_path": str(get_glossary_csv_path()), "term_count": len(glossary), "matching_policy": "csv_exact_match_only", "scope_policy": "llm_based_scope_classifier"}
