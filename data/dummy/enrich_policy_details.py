...
def is_non_cash_amount_evidence(source: str, evidence: str) -> bool:
...
def update_amount_extracted(policy_id: str, amount: int, source: str, evidence: str) -> bool:
    if is_non_cash_amount_evidence(source, evidence):
        print(f"  → 비현금성 한도 제외 [{policy_id}]")
        return update_amount_status(
            policy_id,
            "needs_review",
            "융자·대출·이차보전 또는 비현금성 서비스 한도는 max_amount에 저장하지 않음",
        )
    try:
