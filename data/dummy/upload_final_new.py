...
_amount_llm_cache: dict[str, Any] | None = None


def filter_validation_payload(payload: dict[str, Any]) -> dict[str, Any]:
    """Keep only columns that currently exist in policy_validation_new."""
    return {
        key: value
        for key, value in payload.items()
        if (
            not key.startswith("_")
            and key in VALIDATION_PAYLOAD_FIELDS
            and key not in EXCLUDED_COLLECTION_FIELDS
        )
    }


def detect_non_cash_amount(text: str, amount_type: str = "") -> dict[str, Any] | None:
    """Return review metadata when a number is not a cash grant."""
    compact = clean_text(text, 12000)
    detected_type = ""
    for keyword, amount_label in NON_CASH_AMOUNT_KEYWORDS.items():
        if keyword in compact:
            detected_type = amount_label
            break

    if amount_type == "loan" and not detected_type:
        detected_type = "융자한도"

    if not detected_type:
        has_non_cash_service = any(keyword in compact for keyword in NON_CASH_SERVICE_KEYWORDS)
        has_cash_support = any(keyword in compact for keyword in CASH_SUPPORT_KEYWORDS)
        if has_non_cash_service and not has_cash_support:
            detected_type = "비현금성 서비스 한도"

    if not detected_type:
        return None

    return {
        "임시지원유형": "융자" if "융자" in detected_type or "자금" in detected_type or "대출" in detected_type else "비현금지원",
        "임시금액유형": detected_type,
        "ROI직접차감가능": False,
        "검토필요": True,
        "검토사유": "현금성 최대지원금으로 확정할 수 없어 공통 금액 컬럼에서 제외",
    }


def load_amount_llm_cache() -> dict[str, Any]:
    global _amount_llm_cache
    if _amount_llm_cache is not None:
        return _amount_llm_cache

    empty = {"version": 1, "by_policy": {}, "by_context": {}}
    try:
        loaded = json.loads(AMOUNT_LLM_CACHE_PATH.read_text(encoding="utf-8"))
    ...
    loaded.setdefault("by_policy", {})
    loaded.setdefault("by_context", {})
    _amount_llm_cache = loaded
    return loaded


def amount_context_hash(text: str) -> str:
    normalized = re.sub(r"\s+", " ", clean_text(text, 3000)).strip().lower()
    return hashlib.sha256(normalized.encode("utf-8")).hexdigest()


def cached_amount_result(policy_id: str, context_hash: str) -> dict[str, Any] | None:
    cache = load_amount_llm_cache()
    policy_entry = (cache.get("by_policy") or {}).get(policy_id)
    if (
        isinstance(policy_entry, dict)
        and policy_entry.get("context_hash") == context_hash
        and isinstance(policy_entry.get("result"), dict)
    ):
        return policy_entry["result"]

    context_entry = (cache.get("by_context") or {}).get(context_hash)
...
def save_amount_result(
    ...
    cache["by_policy"][policy_id] = entry
    cache["by_context"][context_hash] = {
        "result": result,
        "updated_at": entry["updated_at"],
    }

    AMOUNT_LLM_CACHE_PATH.parent.mkdir(parents=True, exist_ok=True)
    temp_path = AMOUNT_LLM_CACHE_PATH.with_suffix(
        AMOUNT_LLM_CACHE_PATH.suffix + ".tmp"
    )
    temp_path.write_text(
        json.dumps(cache, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    temp_path.replace(AMOUNT_LLM_CACHE_PATH)
...
def extract_amount_info_with_llm(
    policy_id: str,
    source: str,
) -> dict[str, Any] | None:
    if not llm or not HumanMessage or not SystemMessage:
        return None
    if getattr(llm, "rate_limited", False):
        return None

    compact_source = clean_text(source, 3000)
    context_hash = amount_context_hash(compact_source)
    cached = cached_amount_result(policy_id, context_hash)
    if cached:
        print(f"  - 지원금 Gemini 캐시 재사용: {policy_id}")
        return cached
...
"""
    ...
    try:
        ...
        save_amount_result(policy_id, context_hash, result)
        return result
    except Exception as exc:
...
def extract_amount_info(item: dict[str, Any], detail_text: str) -> dict[str, Any]:
    policy_id = clean_text(item.get("pblancId")) or "unknown"
